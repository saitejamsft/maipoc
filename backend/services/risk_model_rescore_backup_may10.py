...
# "abfss://maipocaa@maipocaa.dfs.core.windows.net",
# "/maipocaa",
# {"linkedService":"maipocaa_stgact"}
# )
...
model = load_model(f"files/Risk_RF_Final")
...
features = pd.read_csv('files/Features.csv')
...
def Target_Encode_Multiclass(X, class_objects):
    ...
    for class_obj in class_objects:
        ...
        infile = open(f"files/pkl/{class_obj}", 'rb')
...
def score(inp, class_objects, jobId, model, selected_features, nominal_vars, ordinal_vars, num_vars, dummy_vars, features):
    ...
    # Creating calculated fields and imputing nulls in new fields
    ...
    df_num_imp = get_data('files/df_num_imp.csv', 'csv')
    ...
    return predict_model(model, dataset_train).to_dict('records')
